...
const TEST_VISIT_RECORD = {
  ...
  startTime: '10:00',
  endTime: '11:30',
  content: '新商品のプレゼンテーションを実施。先方の反応は良好で、次回は見積書を持参することで合意。',
};
...
test.describe('日報作成フロー', () => {
  ...
  test('日報作成から訪問記録・Problem・Plan追加までの一連の流れ', async ({ page }) => {
    ...
    // Step 3: 訪問記録を追加
    // 訪問記録セクションを探す
    const visitSection = page.locator('section, div').filter({ hasText: /訪問記録/ }).first();
    ...
    // 訪問内容
    const contentTextarea = dialog.locator('textarea[name*="content"], textarea[name*="visitContent"]');
    ...
    // Step 4: Problemを追加
    // Problemセクションを探す
    const problemSection = page.locator('section, div').filter({ hasText: /Problem|課題|相談/ }).first();
    ...
    // Step 5: Planを追加
    // Planセクションを探す
    const planSection = page.locator('section, div').filter({ hasText: /Plan|明日の予定|予定/ }).first();
    ...
    const saveReportButton = page.locator('button[type="submit"]').filter({ hasText: /保存/ }).last();
    ...
    // Step 7: 一覧画面に戻る
    // 一覧に戻るボタンをクリック
    const backButton = page.getByRole('link', { name: /一覧に戻る|戻る|一覧/ }).or(
      page.getByRole('button', { name: /一覧に戻る|戻る|一覧/ })
    );
    ...
    // 一覧画面では件数が表示される想定
    const visitCount = page.locator('text=/訪問.*1|1.*訪問/').or(page.locator('td, div').filter({ hasText: '1' }));
    const problemCount = page.locator('text=/Problem.*1|1.*Problem/').or(page.locator('td, div').filter({ hasText: '1' }));
    const planCount = page.locator('text=/Plan.*1|1.*Plan/').or(page.locator('td, div').filter({ hasText: '1' }));

    // 少なくとも1つの件数表示があることを確認
    await expect(
      visitCount.first().or(problemCount.first()).or(planCount.first())
    ).toBeVisible();
  });
  ...
  test.afterEach(async ({ page }) => {
    ...
    try {
      ...
      const todayReport = page.locator(`tr, .report-item`).filter({ hasText: japaneseDate }).first();
