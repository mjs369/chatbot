...
test.describe('コメントフローE2Eテスト', () => {
  ...
  test.beforeAll(async () => {
    ...
    // テスト用の日報を作成
    dailyReportId = await createTestDailyReport(
  ...
  test('上長が部下の日報にコメントを追加できる', async ({ page }) => {
    ...
    await expect(
      page.locator('h1, h2').filter({ hasText: '日報詳細' })
    ...
    const problemCommentButton = page
      .locator('button:has-text("コメント")')
      .first();
    ...
    // コメント入力欄にテキストを入力
    const problemCommentTextarea = page
      .locator('textarea[placeholder*="コメント"]')
      .first();
    await problemCommentTextarea.fill(
      'E2Eテスト: 5%までの値引きであれば承認します。'
    );
    ...
    // コメントが表示されることを確認
    await expect(
      page.locator('text=E2Eテスト: 5%までの値引きであれば承認します。')
    ).toBeVisible({ timeout: 10000 });
    ...
    // コメント入力欄にテキストを入力
    const planCommentTextarea = page
      .locator('textarea[placeholder*="コメント"]')
      .nth(1);
    await planCommentTextarea.fill(
      'E2Eテスト: 見積書の内容を事前に確認させてください。'
    );
    ...
    // コメントアイコンが表示されることを確認
    const reportRowWithComment = page
      .locator(`tr:has-text("${testReportDate}")`)
      .first();
